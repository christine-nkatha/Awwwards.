...
from awwwardapp.views import profile
...
class Profileclass(TestCase):
    def setUp(self):
        ...
        self.user.save()
        self.profile=Profile(user=self.user, url='www.mum.com',profile_pic='mum.png' ,bio='tintin',location='kenya',email='mail@gmail.com')
    ...
    def test_save_profile(self):
      self.new_profile=Profile(user=self.kanosa,url='www.mum.com',profile_pic='mum.png',bio='tintin',location='kenya',email='mail@gmail.com')
...
class ProjectTest(TestCase):
  def setUp(self):
    self.user=User.objects.create(username='christine')
    self.user.save()
    self.project=Projects.objects.create(id=1,name='portfolio',description='tintin',project_image='mum.png',urls='www.mum.com',profile=self.user)
  ...
  def test_get_project(self):
    project=Projects.objects.create(id=1,name='portfolio',description='tintin',project_image='mum.png',urls='www.mum.com',profile=self.user)
    self.project.save_project()
...
class RatingsTestClass(TestCase):
  ...
  def test_project_voters(self):
    self.profile=Profile(user=self.user,url='www.mum.com',profile_pic='mum.png',bio='mum',location='kenya',email='mail@gmail.com')
    self.profile.save_profile()
    voters=Ratings.project_votes(self.profile)
    self.assertTrue(len(voters),1)
    self.assertEqual(voters[0].rater.user.username, 'christine')